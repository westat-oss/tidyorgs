...
#'
#' @export
# detect_business <- function(data, id, input, output, 
#                             email = FALSE, 
#                             country = FALSE, 
#                             parent_org = FALSE, 
#                             org_type = FALSE){ 
...
#     dplyr::mutate("{{input}}" := str_replace_all(!!input, "\\.", ""),
...
#   if (missing(email)) {
#     #   # 3a. match by misc. business terms
...
#       dplyr::mutate("{{output}}" := "Misc. Business") %>%
...
#   # 4b. match by misc. business terms
#   matched_by_sector <- data %>%
#     dplyr::filter(!!id %notin% already_classified) %>%
#     tidytext::unnest_tokens(words, !!input) %>%
#     dplyr::filter(words %in% sector_terms) %>%
...
#       dplyr::rename_all(~stringr::str_replace_all(.,"\"",""))
#   )
#   # # adding basic covariates for business sector
#   suppressMessages(
...
#         joined_data <- joined_data %>% dplyr::left_join(business_data)
...
detect_business <- function(data, id, input, output, output_email,
                            email = FALSE, 
                            country = FALSE, 
                            parent_org = FALSE, 
                            org_type = FALSE){ 
  ...
  if (missing(email)) {
    ...
    matched_by_email <- data %>%
      dplyr::filter(!!id %notin% already_classified) %>%
      email_to_sectors(!!id, !!email, !!output_email, "business") %>%
