...
# detect_academic <- function(data, id, input, output
#                         email = FALSE, 
#                         country = FALSE, 
#                         parent_org = FALSE, 
#                         org_type = FALSE){ 
...
detect_academic <- function(data, id, input, output, output_email,
                        email = FALSE, 
                        country = FALSE, 
                        parent_org = FALSE, 
                        org_type = FALSE){ 
  ...
  #   # 5a. join classified data back to the original dataframe
    suppressMessages(
     ...
     matched_by_email <- data %>%
        dplyr::filter(!!id %notin% already_classified) %>%
        email_to_sectors(!!id, !!email, !!output_email, "academic") %>%
